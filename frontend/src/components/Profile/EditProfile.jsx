...
function EditProfile() {
  ...
  const hobbies = ["Cricket", "Dancing", "Reading", "Riding", "Swimming"]
  ...
  const handleInputChange = ({ name, value, files = '' }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(errors).length == 0) {
      try {
        ...
        formData.append("hobbies", formFields.hobbies)

        Object.keys(formFields.files).forEach(key => {
          formData.append('files[]', formFields.files[key])
        });

        const userId = user.id
        const { data } = await authService.updateProfile(userId, formData);
            ...
            >
              {
                roles.length > 0 ?
                  roles.map(({ id, role_name }, key) => (
                    <Option key={id} value={id.toString()}>{role_name}</Option>
                  ))
                :
                  ...
                  {code.map(({countryCodes, isoCode2}, index) => {
                    return (
                      <MenuItem
                        ...
                        onClick={(event) => handleInputChange(event.target)}
                      >
