...
import { RoleDataType, UserDataType, ValidationErrorType, stateType } from "../../Types";
...
const EditProfile = () => {
  const user: UserDataType = useSelector((state: stateType) => state.user.user);
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(errors).length == 0) {
      try {
        ...
        const { data } = await authService.updateProfile(userId ?? 0, formData);
