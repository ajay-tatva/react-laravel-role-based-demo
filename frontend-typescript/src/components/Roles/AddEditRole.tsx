...
import { RoleDataType, ValidationErrorType } from "../../Types";
...
const AddEditRole = () => {
  const [formTitle, setFormTitle] = useState<string>("Add Role")
  const [formFields, setFormFields] = useState<RoleDataType>({
    role_name: "",
  });
  ...
  const handleInputChange = ({ name, value }: HTMLInputElement) => {
    setFormFields({
      ...formFields,
      [name]: value,
    });
