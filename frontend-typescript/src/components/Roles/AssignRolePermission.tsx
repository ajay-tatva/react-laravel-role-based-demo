...
import { PermissionDataType, RoleDataType } from "../../Types";
...
const AssignRolePermission = () => {
  ...
  const [permissions, setPermissions] = useState<PermissionDataType[]>([]);
  ...
  const getRoles = useCallback(async () => {
    try {
      ...
      if (data.flag) {
        ...
        let permissionData: PermissionDataType[] = [];
        data.data.roles.map((role: RoleDataType, key: number) => {
          permissionData[key] = {
            role_id: role.id ?? 0,
  ...
  const checkRolePermission = (roleId: number, permissionName: string) => {
  ...
  const handleCheckBoxChange = (roleId: number, permissionName: string) => {
    let targetObject: PermissionDataType = permissions.find(
      (permission) => permission.role_id === roleId
